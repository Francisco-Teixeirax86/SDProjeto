...
#include "entry.h"
#include "tree.h"
#include "serialization.h" //remover depois, teste
...
char** testSerialization() {
	char **keys = malloc(sizeof(char*)*4);
	char **buffer;
	keys[0] = "abc";
	keys[1] = "bcd";
	keys[2] = "cde";
	keys[3] = "def";
	keyArray_to_buffer(keys, buffer);
	return buffer;
}
